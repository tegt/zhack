...
;; Tegt key-bindings
(fset 'php-widen
   "\C-xnw\C-[xhtml mo\C-i\C-m")

(global-set-key [?\C-c ?h] 'php-widen)

(fset 'gwt
   "\C-s*/\C-a\C-w\C-o*  @author Grant Tegtmeier <Grant@Tegt.com>\C-m*  @package keypartner\C-m*  @copyright Copyright (C)\C-?\C-?c) 2013 Grant Tegtmeier for Eli\C-?\C-?\C-?r\C-?eLimn LLC\C-x\C-s")

(global-set-key [?\C-c ?c] 'php-widen)

(fset 'php-narrow
   [?\C-s ?? ?> ?\C-b ?\C-b ?\C-r ?< ?? ?p ?h ?p ?\C-n ?\C-a ?\C-@ ?\C-s ?? ?> ?\C-a ?\C-n ?\C-x ?n ?n ?\M-x ?p ?h ?p ?- ?m ?o ?d ?e ?\C-m])

(global-set-key [?\C-c ?y] 'php-narrow)

(fset 'git-ediff
   "\C-e\C-r \C-f\C-k\C-[xei\C-?fi\C-?\C-?diff-revi\C-i\C-m\C-y\C-m\C-m\C-mnnnnn")

(global-set-key [?\C-c ?\=] 'git-ediff)

(fset 'shell-clear
   "\C-x1\C-[xshell\C-m\C-xk\C-myes\C-m\C-x2\C-[xshell\C-m")

(global-set-key [?\C-c ?\!] 'shell-clear)
...
(global-set-key [?\C-c ?c] 'git-status)

(global-set-key [?\C-c ?g] 'git-status)
...
(custom-set-faces
 ...
 '(font-lock-comment-face ((((class color) (min-colors 88) (background light)) (:foreground "brightred"))))
 '(font-lock-function-name-face ((((class color) (min-colors 8)) (:background "blue" :foreground "orange" :weight bold))))
 '(font-lock-string-face ((((class color) (min-colors 88) (background light)) (:background "yellow" :foreground "VioletRed4"))))
 '(font-lock-type-face ((((class color) (min-colors 88) (background light)) (:background "green" :foreground "black"))))
 '(font-lock-variable-name-face ((((class color) (min-colors 88) (background light)) (:foreground "brightgreen"))))
